...
int SimpleFS_remove(DirectoryHandle* d, char* filename) {
	...
	// TODO: gestire il caso in cui si tratti di una cartella
	...
	FileHandle * file_handle = malloc(sizeof(FileHandle));
	file_handle = SimpleFS_openFile(d, filename);
	...
	if(file_handle != NULL) {
		...
		file = file_handle->fcb;
		current_block = file_handle->fcb->fcb.block_in_disk;

		// Se il file ha un blocco successivo
		if(file_handle->fcb->header.next_block != -1) {

			// Fino a quanto ha blocchi successivi, cancello i suoi blocchi
			do {
				next_block = file->header.next_block;
