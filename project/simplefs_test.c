...
int main(int agc, char** argv) {
	...
	printf("\n\n+++ Test DiskDriver_writeBlock()"); 
	printf("\n    Il risultato della write con block_num=%d è %d",0, DiskDriver_writeBlock(&disk,"Ciao",0));
	...
	printf("\n+++ Test DiskDriver_freeBlock()");
	printf("\n    Libero il blocco %d, la funzione ritorna: %d \n",0,DiskDriver_freeBlock(&disk,0));


	// Test DiskDriver_readBlock        
	printf("\n+++ Test DiskDriver_readBlock()");  
	void* dest;
	printf("\n    Il risultato della read con block_num=%d è: %d\n",0, DiskDriver_readBlock(&disk,dest,0));

 
	// Test DiskDriver_writeBlock  
	printf("\n\n+++ Test DiskDriver_writeBlock()"); 
	printf("\n    Il risultato della write con block_num=%d è %d",0, DiskDriver_writeBlock(&disk,"Dracarys",0));
