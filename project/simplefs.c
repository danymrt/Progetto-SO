...
// TODO: QUESTO IF è GIUSTO, CONTROLLO L'ELSE
...
// TODO: Controllare l'if seguente. L'else già funziona
						if(fdb2->header.next_block != -1) {
							for(j = 0; j < sizeof(fdb2->file_blocks); j++) {
							...
							printf("\nci sono %d files", fdb2->num_entries);
							for(j = 0, remaining_entries = fdb2->num_entries; j < sizeof(fdb2->file_blocks); j++) {
								// Se l'elemento contiene un blocco figlio
								if(fdb2->file_blocks[j] > 0 && remaining_entries) {
									// Memorizzo le informazioni del suo figlio
									FirstFileBlock * file_to_delete = malloc(sizeof(FirstFileBlock));
									DiskDriver_readBlock(d->sfs->disk, file_to_delete, fdb2->file_blocks[j]);
									printf("\nprovo a cancellare %s", file_to_delete->fcb.name);
