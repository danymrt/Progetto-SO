...
int SimpleFS_remove(DirectoryHandle* d, char* filename) {
	...
	int i, j, next, ret = -1, previous_block, current_block;
	...
	for(i = 0, j = 0; i < dcb->num_entries; i++, j++){
		if(j >= sizeof(dcb->file_blocks)) { 
			DirectoryBlock * dcb;
			j = j - sizeof(dcb->file_blocks);
			DiskDriver_readBlock(d->sfs->disk, dcb, dcb->header.next_block);
		}
		DiskDriver_readBlock(d->sfs->disk, f, dcb->file_blocks[i]);
		current_block = dcb->file_blocks[i];
		printf("\n%s è %s?", f->fcb.name, filename);
		if(strcmp(f->fcb.name, filename) == 0){
			printf("\n  Si!");
			if(f->fcb.is_dir == 0){
				printf("\n      previous: %d, next: %d, index: %d", f->header.previous_block, f->header.next_block, f->header.block_in_file);
				if(f->header.next_block != -1) {
					printf("\n      Il file ha più di un blocco");
					while(f->header.next_block != -1){
						FileBlock * f = malloc(sizeof(FileBlock));
						current_block = f->header.next_block;
						DiskDriver_readBlock(d->sfs->disk, f, current_block);
					}
					// In questo punto abbiamo, dentro "f", l'ultimo blocco del file
					while(f->header.previous_block != -1) {
						previous_block = f->header.previous_block;
						DiskDriver_freeBlock(d->sfs->disk, current_block);
						DiskDriver_readBlock(d->sfs->disk, f, previous_block);
						current_block = previous_block;
					}
				}else{
					printf("\n      Il file ha un solo blocco");
